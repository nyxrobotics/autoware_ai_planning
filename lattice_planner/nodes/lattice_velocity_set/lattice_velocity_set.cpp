...
ros::Publisher g_local_waypoints_pub;
...
void PathVset::setTemporalWaypoints()
{
  ...
  local_waypoints_.increment = current_waypoints_.increment;
  // push current pose
  ...
  local_waypoints_.waypoints.push_back(current_point);
  for (int i = 0; i < size; i++)
  {
    if (g_closest_waypoint + i >= getSize())
      return;
    local_waypoints_.waypoints.push_back(current_waypoints_.waypoints[g_closest_waypoint + i]);
  }
...
void changeWaypoint(EControl detection_result)
{
  ...
  {  // STOP for obstacle
    ...
    g_local_waypoints_pub.publish(g_path_change.getTemporalWaypoints());
  }
...
int main(int argc, char** argv)
{
  ...
  g_local_waypoints_pub = nh.advertise<autoware_msgs::Lane>("local_waypoints", 1000, true);
